...
std_msgs::UInt16MultiArray pot_array_raw; //Raw POT values from ADC 
...
std_msgs::UInt16MultiArray fsr_array_raw;         //Raw FSR values from ADC
...
ros::Publisher pot_pub("potentiometer_raw", &pot_array_raw);
ros::Publisher pot_proc("potentiometer_processed", &pot_array);
ros::Publisher fsr_pub("fsr_raw", &fsr_array_raw);
ros::Publisher fsr_proc("fsr_processed", &fsr_array);
...
void loop(){
  ...
  u16 temp = 0;
	...
	for(int i = 0; i < 9; i++){
    	if(temp = analogRead(POT1+i))
      {}
      else
      temp = 0;
      temp = rand() % 90;
		pot_array_raw.data[i] = temp;
    //msg_pub1.data = temp;
    //chatter_pub1.publish(&msg_pub);		
		pot_array.data[i] = temp*333.0/1024.0; //Conversion function detailed in Datasheet
		}
